      ...
      <div className="mx-auto max-w-[1600px]">
        ...
        <section className="mt-5 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <form method="get" className="mt-3 grid gap-3 md:grid-cols-3 xl:grid-cols-6">
            <label className="text-xs font-medium text-slate-700">Account<select name="account" defaultValue={accountFilter} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-2 py-1.5 text-xs"><option value="ALL">All accounts</option>{accounts.map((account) => <option key={account.code} value={account.code}>{account.name}</option>)}</select></label>
            <label className="text-xs font-medium text-slate-700">Category<select name="category" defaultValue={categoryFilter} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-2 py-1.5 text-xs"><option value="ALL">All categories</option>{ACCOUNT_CATEGORIES.map((category) => <option key={category} value={category}>{category}</option>)}</select></label>
            ...
            <label className="text-xs font-medium text-slate-700">From<input type="date" name="from" defaultValue={params.from ?? ""} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-xs" /></label>
            <label className="text-xs font-medium text-slate-700">To<input type="date" name="to" defaultValue={params.to ?? ""} className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-xs" /></label>
            <label className="text-xs font-medium text-slate-700">Description<input type="search" name="search" defaultValue={params.search ?? ""} placeholder="Search..." className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-xs" /></label>
        ...
        <form action={saveTransactionCategories} className="mt-5 rounded-xl border border-slate-200 bg-white shadow-sm">
          <input type="hidden" name="returnTo" value={currentUrl} />
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 p-3">
            <div><h2 className="text-sm font-semibold text-slate-900">Transaction history</h2><p className="mt-0.5 text-xs text-slate-500">{filteredRows.length === 0 ? "No transactions" : `Showing ${start + 1}-${Math.min(start + PAGE_SIZE, filteredRows.length)} of ${filteredRows.length}`}. {uncategorised} uncategorised. Tick Delete for any rows you want removed, then Save all.</p></div>
            <div className="flex items-center gap-4 text-xs">
              <span className="font-semibold text-emerald-700">Credits {money(filteredCredit)}</span><span className="font-semibold text-red-700">Debits {money(filteredDebit)}</span><span className="font-semibold text-slate-900">Net {money(filteredCredit - filteredDebit)}</span>
              {visibleRows.length > 0 && <button type="submit" className="rounded-md bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-800">Save all</button>}
            </div>
          </div>

          {filteredRows.length === 0 ? <div className="p-6 text-center text-xs text-slate-500">No transactions match the selected filters.</div> : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full table-fixed divide-y divide-slate-200 text-[11px] leading-tight">
                  <colgroup><col className="w-[6%]" /><col className="w-[8%]" /><col className="w-[10%]" /><col className="w-[22%]" /><col className="w-[7%]" /><col className="w-[7%]" /><col className="w-[8%]" /><col className="w-[17%]" /><col className="w-[9%]" /><col className="w-[6%]" /></colgroup>
                  <thead className="bg-slate-50"><tr>{["Date","Account","Category","Description","Credit","Debit","Balance","Amend category","Direction","Delete"].map((heading) => <th key={heading} className="px-2 py-2 text-left text-[10px] font-semibold uppercase text-slate-500">{heading}</th>)}</tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {visibleRows.map((row) => (
                      <tr key={row.id} className={row.category === "Uncategorised" ? "bg-amber-50" : ""}>
                        ...
                        <td className="px-2 py-1 text-center"><input type="checkbox" name="deleteTransaction" value={row.id} aria-label={`Delete ${row.description}`} className="h-4 w-4 rounded border-slate-300" /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex items-center justify-between gap-3 border-t border-slate-200 p-3 text-xs">
                ...
                <div className="flex items-center gap-2">
                  <button type="submit" className="rounded-md bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-800">Save all</button>
